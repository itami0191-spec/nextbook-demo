...
const flowRe =
  /function titlesReadingFlow\(matchedBooks, max = 3\) \{[\s\S]*?\.join\("[^"]*"\);\n\}\n\nfunction phrasePick/;
...
function phrasePick`;
if (!flowRe.test(s)) throw new Error("flow not found");
s = s.replace(flowRe, flowNeu);

const anchorRe =
  /DOM\.anchorNote\.textContent = `[^`]*$/m;
